...
The CNN has been rewritten in C++ code, and synthesized and exported as a Vivado IP using Vitis HLS for acceleration in the PL.

The C++ code for the accelerated CNN can be found in the `CNN\hls_implementation` folder.

Data transfer using DMA is implemented using DMA Streams. An alternative implementation using AXI-Lite is also given. The two implementations can both be found in this folder.

### CNN IP

The CNN is exported as an IP to: `ip/infer.zip`

## Implementing the CNN in Vivado

The CNN IP is implemented in Vivado using the block design: `hdl/projects/u96v2_sbc_mp4d_2020_2/u96v2_sbc_mp4d.xpr`

Running the `mp4d-setup` command should generate the block design with CNN IP and AXI DMA block already setup.
...
The [MPSoC4Drones Readme](#mpsoc4drones) explains how to further deploy on the MPSoC.
